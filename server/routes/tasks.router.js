...
router.delete('/:id', (req, res)=> { //make sure route is not /tasks/:id, we only we use /:id because we are only using the tasks route
    ...
    pool.query(queryText, [Number(idToDelete)])// has to be in an array everytime & since it is expecting a number, it was sending as a string so we need 
    .then ((results) => {
